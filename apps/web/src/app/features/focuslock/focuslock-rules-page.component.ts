            ...
            <li class="fl-rule-item" [class.fl-rule-item--inactive]="!rule.active">
              <div class="fl-rule-item__head">
                ...
                <div class="fl-rule-item__actions">
                  <button type="button" class="fl-rule-action-btn" [class.fl-rule-action-btn--off]="!rule.active" (click)="toggleRule(rule)">
                    {{ rule.active ? 'ON' : 'OFF' }}
                  </button>
                  <button type="button" class="fl-rule-action-btn" (click)="editRule(rule)">
                  ...
                  <button type="button" class="fl-rule-action-btn fl-rule-action-btn--danger" (click)="deleteRule(rule)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>
                  </button>
                </div>
              </div>
              @if (!rule.active) {
...
export class FocuslockRulesPageComponent implements OnInit {
  ...
  protected toggleDay(key: string): void {
    this.form.scheduleDays = { ...this.form.scheduleDays, [key]: !this.form.scheduleDays[key] };
  }
